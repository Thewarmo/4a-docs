...
public class ProductsController {
    ...
    @GetMapping("/productosName/{nombre_producto}")
    ArrayList<Productos> getNomProducto(@PathVariable String nombre_producto){
        ArrayList<Productos> lista = new ArrayList<>();
        ArrayList<Productos> lProductos = new ArrayList<>();
        Productos producto = null;
        lista.addAll(productosRepository.findAll());
        for (Productos p: lista) {
            if(p.getNombre_producto().equals(nombre_producto)){
                producto = new Productos(p.getIdProducto(),p.getNombre_producto(),p.getPrecio(),p.getCantidad(),p.getImagen());
